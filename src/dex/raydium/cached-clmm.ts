...
import { getRayClmmPool, getRayClmmPriceInfo } from "./clmm";
...
export async function getCachedRayClmmPriceInfo(ca: string, poolId?: string) {
  try {
    // Check if we have cached pool data
    const cachedPool = poolId 
      ? RaydiumCache.getCachedPools(ca, 'clmm').find(p => p.poolId === poolId)
      : RaydiumCache.getBestPool(ca, 'clmm');

    if (cachedPool) {
      // Use the original function with poolId for efficiency
      const result = await getRayClmmPriceInfo(ca, cachedPool.poolId);
    ...
    // Get fresh pool data
    const freshPoolInfo = await getRayClmmPool(ca, poolId) as any;
    ...
    RaydiumCache.storePoolInfo(ca, 'clmm', poolsToCache);
    
    // Now get the price using the fresh data
    return await getRayClmmPriceInfo(ca, poolId);
...
export async function populateClmmPoolCache(ca: string) {
  try {
    // Get all pools for this token
    const allPools = await getRayClmmPool(ca, undefined, true);
    ...
    if (allPools && 'pools' in allPools) {
      ...
      for (let i = 0; i < allPools.pools.length; i++) {
        try {
          ...
          const poolInfo = await getRayClmmPool(ca, poolId) as any;
    ...
    console.error(`[CACHE] Error populating CLMM cache for ${ca}:`, error);
    throw error;
  }
} 